...
// int *p = &i;
    /*
        1.定义一个指针变量p，保存变量i的地址,p指向变量i
        2.p保存地址，i保存整型，本质都是变量
        3.此时*p等同于i
    */
...
// {
//     int temp;
//     t = *p;/*将指针变量p指向的变量的值赋给temp，即将变量a的值赋给temp*/
//     *p = *q;/*将指针变量q指向的变量的值赋给指针变量p指向的变量，即将变量b的值赋给变量a*/
//     *q = t;/*将temp的值赋给指针变量q指向的变量，即将temp的值赋给变量b*/
//     return 0;
// }
...
// }
    ...
    /*
    函数通过两个参数输入，来确定一个一维数组,数组的第一个元素首地址，还有长度
    void f(int *a, int length){
    int i;
    for (i = 0; i < length; i++){
        printf("%d ", *(a + i));输出指针a指向的变量的值，即数组a的第i个元素的值
    } 
    定义一个函数f，参数是一个数组a，数组长度length
    }
    此时f函数可以输出任何一个一维数组的内容
    
    */

/*一个指针变量，无论指向的变量占据多少字节，这个指针变量自己本身只占4个字节*/


/*
    动态内存分配：
        传统数组缺点：
            1.数组长度必须事先确定，常量整数，不能改变
            2.传统形式定义的数组，该数组的内存程序员无法手动释放
              数组一旦定义，系统为其分配的内存空间，程序员无法手动释放，直到程序结束，系统才会自动释放
            3.数组长度不能在函数运行过程中动态扩充或者缩小  
            4.A函数定义的数组，在A运行期间可以被其他函数使用，
              但是A函数运行结束后，数组的内存空间就会被系统自动释放，其他函数无法再使用该数组
*/


/*54. 字符串*/

// char word[] = {'g', 'o', 'o', 'd', '\0'};/*字符串以'\0'结尾，表示字符串结束*/

// /*1. 字符串变量*/
// char *str = "good";/*字符串变量，指向字符串常量"good"的首地址*/
// char word[] = "good";/*字符串变量，指向字符串常量"good"的首地址*/
// char word[10] = "good";/*字符串变量，指向字符串常量"good"的首地址*/

/*区别*/
// char *str = "good";/*字符串变量，指向字符串常量"good"的首地址，不能修改字符串内容,构造一个字符串用*/
// char word[] = "good";/*字符串变量，指向字符串常量"good"的首地址，可以修改字符串内容,修改一个字符串用*/

/*字符串的输入输出*/
// char str[10];
// scanf("%s", str);/*输入字符串，不能输入空格，遇到空格结束*/
// printf("%s\n", str);/*输出字符串*/
// printf("%8s\n", str);/*输出字符串，占8个字符宽度*/
...
/*字符串数组*/
    ...
    char a[][10] = {"hello", "world", "!"};/*定义一个字符串数组，包含3个字符串，每个字符串最多包含9个字符，最后一个字符为'\0'*/
    /*指的是，a[0] = char [10]，在数组a的[0]位置上放的是一个char数组，char数组有10个元素*/
    char *b[] = {"hello", "world", "!"};/*定义一个字符串指针数组，包含3个字符串，每个字符串最多包含9个字符，最后一个字符为'\0'*/
    /*b[0] = char * */
    return 0;
}